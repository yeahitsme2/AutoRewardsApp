...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const accessToken = (req.body?.access_token as string | undefined) ?? null;
    const tokenForUser = accessToken || (typeof authHeader === 'string' ? authHeader.replace('Bearer ', '') : null);
    if (!tokenForUser) {
    ...
    const label = req.body?.label ?? 'Custom';
    const periodType = req.body?.period_type ?? 'custom';
    const requestedShopId = req.body?.shop_id ?? null;
    ...
    if (!shopId) {
      res.status(400).json({ error: 'Missing shop context' });
      return;
    }
