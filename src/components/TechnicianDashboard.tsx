...
export function TechnicianDashboard() {
  ...
  const selectedRo = useMemo(
    () => repairOrders.find((ro) => ro.id === selectedRoId) || null,
    [repairOrders, selectedRoId]
  );

  const selectedCustomerName = useMemo(() => {
    if (!selectedRo) return null;
    if (selectedRo.temp_customer_name) return selectedRo.temp_customer_name;
    if (selectedRo.customer_id && customerNameById[selectedRo.customer_id]) {
      return customerNameById[selectedRo.customer_id];
    }
